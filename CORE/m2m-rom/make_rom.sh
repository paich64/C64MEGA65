...
vd_count=$(awk '/constant C_VDNUM/ {gsub(/.*:=|;.*/, "", $0); split($0, a, " "); print a[1]}' ../vhdl/globals.vhd)
crtrom_man_count=$(awk '/constant C_CRTROMS_MAN_NUM/ {gsub(/.*:=|;.*/, "", $0); split($0, a, " "); print a[1]}' ../vhdl/globals.vhd)
...
print_file_handles "HNDL_VD_FILES" "HANDLE_VD_FILE" $vd_count
print_file_handles "HNDL_RM_FILES" "HANDLE_RM_FILE" $crtrom_man_count
